...
de los ambientes (condiciones). 
    ...
    ```{r, fig.cap="Caracterizacion de los ambientes en el plano principal AFM."}
...
variable-ambiente. 
    ...
    ```{r, fig.width=7, fig.height=4, fig.cap="Configuracion "}
    ggplot(parciales) + 
      geom_vline(xintercept = 0, linetype = "dashed", color = "gray30", alpha = 0.5) + 
      geom_hline(yintercept = 0, linetype = "dashed", color = "gray30", alpha = 0.5) +
      geom_segment(
    ...
    ```{r, fig.width = 6, fig.height = 6, out.width="60%", fig.cap="Caracterizacion de las variables en el plano principal AFM."}
    ...
    puede ver que muchos genotipos presentan caracterizaciones muy distintas
    en ambos ambientes. A simple vista, uno puede resaltar a las variedades
    ...
    entre ellas en el plano principal. 
    ...
    forman un angulo aproximado de 90
...
A) ¿Cuáles son las variables que más contribuyen a la determinación de los ejes? 
    ...
    Para determinar que presentan la mayor contribucion con los ejes vamos a tomar
    como referencia a la maxima correlacion, y quedarnos con aquellas variables
    cuya correlacion sea al menos dos tercios de esta cantidad. 
    ...
    el peso de 1000 granos, y el rendimiento, para ambos Ambientes. El hecho de
    que para ambos ambientes tengamos las mismas variables altamente correlacionadas
    con la primera dimension nos indica que la relacion entre estas variables 
    es consistente en ambos ambientes.
    ...
    Ferre es consistente con lo que observamos en el punto C, donde vimos
    que la dimensionalidad para el ambiente Ferre era mayor que para el 
    ambiente Pergamino. Esto se da, principalmente, porque en el ambiente
...
principal del AFM. Compare dicha matriz con la obtenida a partir de APG. 
    ...
    ggplot(concordancia_df) + 
      ...
      labs(
          ...
          y = "Distancia en el plano principal de AOG"
      )
    
    ```
    
    La correlacion entre las distancias en los planos principales es de 
    `r round(cor(dist_mfa, dist_gpa), 3)`
