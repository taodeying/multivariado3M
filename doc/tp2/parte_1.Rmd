...
A) ¿Cuáles son las variables que más contribuyen a la determinación de los ejes? 
  ...
  Usar la mayor correlacion con el eje, y quedarnos con las variables que tengan
  carga mayor a 2/3 el valor maximo.
    ...
    coord[abs(coord$Dim.1) > 0.75 * max_dim1, c("Dim.1", "ambiente", "variable")]
    coord[abs(coord$Dim.2) > 0.75 * max_dim2, c("Dim.2", "ambiente", "variable")]
    ```
...
ejercicio y comente el resultado (Usar UPGMA). 
    ...
    ```{r}
    #HCPC(datos, method = "average") # datos o mfa?
    ```
